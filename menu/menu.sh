...
BURIQ () {
    curl -sS https://raw.githubusercontent.com/jamcbanbkn/tarap/main/ipvps > /root/tmp
...
Name=$(curl -sS https://raw.githubusercontent.com/jamghbanbkn/tarap/main/ipvps | grep $MYIP | awk '{print $2}')
...
PERMISSION () {
    MYIP=$(curl -sS ipv4.icanhazip.com)
    IZIN=$(curl -sS https://raw.githubusercontent.com/jamgbanbkn/tarap/main/ipvps | awk '{print $4}' | grep $MYIP)
    if [ "$MYIP" = "$IZIN" ]; then
    Bloman
    else
...
Exp=$(curl -sS https://raw.githubusercontent.com/jambagbkn/tarap/main/ipvps | grep $MYIP | awk '{print $3}')
fi
export RED='\033[0;31m'
...
Exp1=$(curl https://raw.githubusercontent.com/jambagnbkn/tarap/main/ipvps | grep $MYIP | awk '{print $4}')
if [[ $today < $Exp1 ]]; then
    sts="${Info}"
else
    sts="${Error}"
...
sleep 2
wget -q -O /root/install_up.sh "https://raw.githubusercontent.com/arlanstore/24clanSSHPro/main/menu/install-up.sh" && chmod +x /root/install_up.sh
sleep 2
./install_up.sh
sleep 5
rm /root/install_up.sh
rm /opt/.ver
version_up=$( curl -sS https://raw.githubusercontent.com/arlanstore/Profile/main/Profile/permission/versi )
...
serverV=$( curl -sS https://raw.githubusercontent.com/arlanstore/24clanSSHPro/main/menu/versi )
if [ "$Isadmin" = "ON" ]; then
